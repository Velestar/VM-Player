...
Set frame size. The format is B<wxh> (default - same as source).
...
=item B<-passlogfile> I<prefix> B<(>I<global>B<)>
...
=item B<-map [-]>I<input_file_id>B<[:>I<stream_specifier>B<][,>I<sync_file_id>B<[:>I<stream_specifier>B<]] (>I<output>B<)>
...
I<output_file_id>.I<stream_specifier> are not set, the audio channel will
...
You can also extract each channel of an I<INPUT> to specific outputs; the
following command extract each channel of the audio stream (file 0, stream 0)
to the respective I<OUTPUT_CH0> and I<OUTPUT_CH1>:
...
supported for example with the command B<dov4l> for Video4Linux
devices and using B<-list_formats all> for Video4Linux2 devices.
...
is recognized by the C<-vf> option of the ff*
tools, and by the C<avfilter_graph_parse()> function defined in
...
"I<sample_format>:I<channel_layout>:I<packing_format>".

I<sample_format> specifies the sample format, and can be a string or
the corresponding numeric value defined in F<libavutil/samplefmt.h>.
...
I<packing_format> specifies the type of packing in output, can be one
...
"I<sample_formats>:I<channel_layouts>:I<packing_formats>".
...
I<sample_rate>:I<sample_fmt>:I<channel_layout>:I<packing>
...
with or without text parameter. I<rate> option must be specified.
Note that timecode options are I<not> effective if FFmpeg is build with
C<--disable-avcodec>.
